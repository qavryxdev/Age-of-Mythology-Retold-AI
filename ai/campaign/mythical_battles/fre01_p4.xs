...
/* fre01_p4.xs
   ...
   Halogi's Fire Giants (Loki)
   Owns a base in the east where it produces Rock Giants, Fire Giants and a few Longhouse units. Doesn't attack until called upon.
*/
//==============================================================================
// Includes
...
int gFirstLandUnit = cUnitTypeFireGiant; // Gets trained from the start.
float gMaintainFirstLandUnitAmount = 2;
int gSecondLandUnit = cUnitTypeRockGiant; // Gets trained from the start.
float gMaintainSecondLandUnitAmount = 1; // Base value that gets multiplied. They also train 1 more than this value.
int gThirdLandUnit = cUnitTypeThrowingAxeman; // Gets trained from the start.
float gMaintainThirdLandUnitAmount = 4;
int gFourthLandUnit = cUnitTypeHirdman; // Gets trained from the start.
...
bool gInvokeFlamingWeapons = false; // Every attack, we have a percentage chance to get to invoke Flaming Weapons once.
int gFlamingWeaponsChance = 20; // The lower threshold a 100-sided die needs to roll under to succeed.

vector gOurTCLocation = vector(293.0, 0.0, 125.0);
...
Strategy scenarioAttackWaveStrategy()
{
   ...
   strategy.mInitFunc = [](ref StrategyData data) -> bool
   {
      ...
      gMaintainSecondLandUnitAmount += 1; // Train 1 more Rock Giant than the multiplied value.
      ...
      gAttackWave.addAttackUnitType(gFirstLandUnit);  // Fire Giants
      gAttackWave.addAttackUnitType(gSecondLandUnit); // Rock Giants
      gAttackWave.addAttackUnitType(gThirdLandUnit); // Throwing Axemen
      gAttackWave.addAttackUnitType(gFourthLandUnit); // Hirdmen
      ...
      vector startPoint = vector(254.0, 0.0, 120.0); // At the entrance to our base.
      ...
      kbPathAddWaypoint(pathID1, vector(194.0, 0.0, 108.0));
      kbPathAddWaypoint(pathID1, vector(166.0, 0.0, 81.0));
      kbPathAddWaypoint(pathID1, vector(119.0, 0.0, 105.0));
      kbPathAddWaypoint(pathID1, vector(90.0, 0.0, 57.0));
      ...
      kbPathAddWaypoint(pathID2, vector(194.0, 0.0, 108.0));
      kbPathAddWaypoint(pathID2, vector(166.0, 0.0, 81.0));
      kbPathAddWaypoint(pathID2, vector(172.0, 0.0, 46.0));
      kbPathAddWaypoint(pathID2, vector(109.0, 0.0, 40.0));
      ...
      gAttackWave.setWaveStartCallback(
         [](int planID = -1)
         {
            // gFlamingWeaponsChance-percent chance to start using Flaming Weapons.
            // When succeeding a roll, Flaming Weapons gets to be invoked once before we have to roll again.
            if (gInvokeFlamingWeapons == false)
            {
               int random = xsRandInt(99) + 1;
               debugAttackWave("Rolling a die to see if we can invoke Flaming Weapons. The result is... " + random + "!");
               if (random <= gFlamingWeaponsChance)
               {
                  gInvokeFlamingWeapons = true;
                  xsEnableRule("useFlamingWeapons");
                  debugAttackWave("Success! The die roll beats the threshold of " + gFlamingWeaponsChance + "! Now we can invoke Flaming Weapons once before rolling again.");
               }
               else
               {
                  debugAttackWave("No Flaming Weapons invocations yet. The die roll needs to be under " + gFlamingWeaponsChance + " for use to start casting it.");
               }
            }
         }
      );
...
void preInit()
{
   ...
   gMainGatherBase = createOverrideGatherBase(vector(271.00, 0.00, 131.00), 55); // Covering most of our large base.
...
// Called by trigger "O3_Halogi_Open" to enable attacks.
void enableAttacks()
{
...
group ruleGroupRebuild
{
   ...
   vector location = vector(260.0, 0.0, 148.0);
...
group ruleGroupRebuild
{
   ...
   vector location = vector(278.0, 0.0, 144.0);
   if (kbUnitCount(building, cMyID, cUnitStateABQ) < 1)
   {
      buildBuilding(building, location);
   }
