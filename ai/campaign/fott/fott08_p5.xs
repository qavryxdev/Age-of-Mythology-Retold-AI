...
/* fott08_p5.xs
   ...
   Red Greek player owning the base in the east (Gargarensis). Trains Hypaspists,
   Peltasts, Cyclopes, and Nemean Lions to attack Arkantos' base.
...
int gSecondLandUnit = cUnitTypePeltast; // Gets trained from the start.
float gMaintainSecondLandUnitAmount = 7;
int gThirdLandUnit = cUnitTypeCyclops; // Begins training at 600 seconds; train delay of 120.
float gMaintainThirdLandUnitAmount = 2;
int gFourthLandUnit = cUnitTypeNemeanLion; // Begins training at 600 seconds; train delay of 120.
float gMaintainFourthLandUnitAmount = 2;
int gFifthLandUnit = cUnitTypePetrobolos; // Begins training at 480 seconds, but not on Easy.
float gMaintainFifthLandUnitAmount = 2;
int gSixthLandUnit = cUnitTypeHippolyta; // Begins training at 540 seconds; wait 420 to retrain after death.
float gMaintainSixthLandUnitAmount = 1;

float gMaxVillagerCount = 15;
float gAttackStartDelay = 540; // In seconds; starts slightly later than P2's first attack to prevent overlap.
...
{
   xsEnableRule("maintainHippolyta");
   ...
   strategy.mInitFunc = [](ref StrategyData data) -> bool
   {
      ...
      // Cyclopes, Nemean Lions, and Hipolyta not affected by multiplier.
      ...
      {
         gAttackStartDelay = 900; // Don't launch the first attack until well into the mission; it's tricky managing 2 bases.
      ...
      data.addUnitToMaintain(gFirstLandUnit, gMaintainFirstLandUnitAmount); // Hypaspists
      data.addUnitToMaintain(gSecondLandUnit, gMaintainSecondLandUnitAmount); // Peltasts

      // Cyclopes and Nemean Lions are not produced until later.
      
      // Petroboli are not produced until later.
      
      // Hippolyta is not produced until later.
      ...
      gAttackWave.addAttackUnitType(gFirstLandUnit); // Hypaspists
      gAttackWave.addAttackUnitType(gSecondLandUnit); // Peltasts

      // Cyclopes and Nemean Lions only join attack waves on Hard and Titan, and only 10 minutes into the mission.
      // Hippolyta only joins attack waves 9 minutes into the mission.
      ...
      vector startPoint = vector(260.0, 0.0, 89.0); // Terrace west of eastern Settlement.
      vector targetPoint = vector(126.0, 0.0, 38.0); // Arkantos' TC.
      ...
      kbPathAddWaypoint(pathID1, vector(287.0, 0.0, 87.0)); // Block #1
      kbPathAddWaypoint(pathID1, vector(207.0, 0.0, 88.0)); // Block #2
      kbPathAddWaypoint(pathID1, vector(165.0, 0.0, 97.0)); // Block #3
   ...
   // SPLIT AMOUNTS
      int firstLandUnitSplitAmount = gMaintainFirstLandUnitAmount / 3; // Hypaspists
      int secondLandUnitSplitAmount = gMaintainSecondLandUnitAmount / 3; // Peltasts
      int thirdLandUnitSplitAmount = gMaintainThirdLandUnitAmount / 2; // Cyclopes
      int fourthLandUnitSplitAmount = gMaintainFourthLandUnitAmount / 2; // Nemean Lions

   // DEFINE THE PLANS
      // Plan 1 (First production buildings, later moved up)
      gDefendPlan1 = createDefendPlan("Defense Plan 1", kbBaseGetMainID(cMyID), 10, vector(259.0, 0.0, 87.0), 30);
      ...
      aiPlanAddUnitType(gDefendPlan1, gFourthLandUnit, 0, fourthLandUnitSplitAmount, 200); // Lions
      aiPlanAddUnitType(gDefendPlan1, cUnitTypeAtalanta, 0, 0, 200); // Hippolyta

      // Plan 2 (In front of their economy)
      gDefendPlan2 = createDefendPlan("Defense Plan 2", kbBaseGetMainID(cMyID), 10, vector(303.0, 0.0, 175.0), 30);
      ...
      aiPlanAddUnitType(gDefendPlan2, gFourthLandUnit, 0, fourthLandUnitSplitAmount, 200); // Lions
      aiPlanAddUnitType(gDefendPlan2, gFifthLandUnit, 0, 0, 200); // Petroboli
      aiPlanAddUnitType(gDefendPlan2, cUnitTypeAtalanta, 0, 0, 200); // Hippolyta
   ...
   strategy.mUpdateFunc = [](ref StrategyData data) -> bool
   {
      ...
      static bool Added_Lions = false;
      static bool Added_Petroboli = false;
      static bool Added_Hippolyta = false;
      ...
      // Hippolyta
      if (time >= 540 && Added_Hippolyta == false)
      {
         data.addUnitToMaintain(gSixthLandUnit, gMaintainSixthLandUnitAmount);
         data.setTrainDelay(gSixthLandUnit, gTrainDelay);
         gAttackWave.addAttackUnitType(gSixthLandUnit); // Start dispatching Hippolyta.
         Added_Hippolyta = true;
         xsEnableRule("maintainHippolyta");
      }
      ...
      if (cDifficultyCurrent >= cDifficultyModerate)
      {
         ...
         {
            if (Added_Lions == false)
            {
               data.addUnitToMaintain(gFourthLandUnit, gMaintainFourthLandUnitAmount); // Start training Nemean Lions.
               data.setTrainDelay(gFourthLandUnit, gMythTrainDelay);
               gAttackWave.addAttackUnitType(gFourthLandUnit); // Start dispatching Nemean Lions.
         ...
         if (time >= 840 && cDifficultyCurrent >= cDifficultyModerate)
         {
         ...
         if (army_buffed == false && time >= 1500)
         {
            ...
            if (cDifficultyCurrent == cDifficultyModerate)
            {
               gMaintainFirstLandUnitAmount *= 1.15; // Train +15% Hypaspists.
               gMaintainSecondLandUnitAmount *= 1.15; // Train +15% Peltasts.
            ...
            if (cDifficultyCurrent == cDifficultyHard)
            {
               ...
               gMaintainSecondLandUnitAmount *= 1.20; // Train +20% Peltasts.
               ...
               // Train more Villagers to support the larger army.
               gMaxVillagerCount *= 1.25; // Train +50% more Villagers.
               gOverrideMaxVillagerPop = gMaxVillagerCount;
            }
            ...
            if (cDifficultyCurrent == cDifficultyTitan)
            {
               gMaintainFirstLandUnitAmount *= 1.30; // Train +30% Hypaspists.
               gMaintainSecondLandUnitAmount *= 1.30; // Train +30% Peltasts.
               gMaintainThirdLandUnitAmount *= 2.00; // Train +100% Cyclopes.
               gMaintainFourthLandUnitAmount *= 2.00; // Train +100% Nemean Lions.
...
void preInit()
{
   ...
   gMainGatherBase = createOverrideGatherBase(vector(358.00, 0.00, 155.00), 100);
   createOverrideGatherBase(vector(297.00, 0.00, 145.00), 80);
...
minInterval 10
{
   ...
   if (trainHippolyta == true)
   {
      ...
      trainHippolyta = false;
      return;
   }
   if (kbUnitCount(cUnitTypeHippolyta, cMyID, cUnitStateAlive) >= 1)
   {
      // If we have an Hippolyta alive, null out the maintain plan.
   ...
   else
   {
      ...
      {
         // We currently don't have an Hippolyta but we're trying to train her.
         return;
      }
      // Our Hippolyta is dead so now we must retrain her in 7 minutes.
      trainHippolyta = true;
      xsSetRuleMinIntervalSelf(420);
   }
}

// Use Lightning Storm when asked to.
rule useLightningStorm
inactive
minInterval 5
{
   int numEnemies =
   getUnitCountByLocation(cUnitTypeLogicalTypeLandMilitary, 1, cUnitStateAlive, vector(298.0, 0.0, 146.0), 40.0);
   debugAttackWave("numEnemies for casting Lightning Storm: " + numEnemies);
   if (numEnemies >= 5)
   {
      if (aiCastGodPowerAtPosition(cProtoPowerLightningStorm, vector(298.0, 0.0, 146.0)) == true)
      {
         debugAttackWave("Casted Lightning Storm!");
         xsDisableRule("useLightningStorm");
      }
   }
}
...
// Research Roar of Orthus at 1000 seconds; Hard and Titan only.
...
{
   // Arkantos drove us in. Move defend point 1 closer to our Fortress.
   aiPlanSetVariableVector(gDefendPlan1, cDefendPlanTargetPoint, 0, vector(299.0, 5.0, 201.0));
   aiPlanSetVariableVector(gDefendPlan1, cDefendPlanGatherPoint, 0, vector(299.0, 5.0, 201.0));
