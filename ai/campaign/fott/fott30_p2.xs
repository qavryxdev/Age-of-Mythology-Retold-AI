...
int gFirstLandUnit = cUnitTypeBattleBoar; // Gets trained from the start.
float gMaintainFirstLandUnitAmount = 1;
int gSecondLandUnit = cUnitTypeFenrisWolfBrood; // Gets trained from the start.
float gMaintainSecondLandUnitAmount = 2;
int gThirdLandUnit = cUnitTypeJarl; // Gets trained from the start.
float gMaintainThirdLandUnitAmount = 3;
int gFourthLandUnit = cUnitTypeHuskarl; // Gets trained starting from the Mythic Age.
float gMaintainFourthLandUnitAmount = 5;
int gFifthLandUnit = cUnitTypePortableRam; // Gets trained starting from the Mythic Age.
float gMaintainFifthLandUnitAmount = 1;
int gSixthLandUnit = cUnitTypeGodi; // Gets trained starting from the Mythic Age.
float gMaintainSixthLandUnitAmount = 4;
int gSeventhLandUnit = cUnitTypeHirdman; // Gets trained starting from the Mythic Age.
float gMaintainSeventhLandUnitAmount = 7;
...
float gAttackStartDelayLong = cWaitWithAttacking; // In seconds, effectively 'preventing' attacks until function ActivateP2() is called.
...
Strategy scenarioAttackWaveStrategy()
{
   ...
   strategy.mInitFunc = [](ref StrategyData data) -> bool
   {
      ...
      // We must have a second attack wave as well.
      ...
      // Don't train Portable Rams until a bit later.
      ...
      vector startPoint = vector(369.0, 0.00, 131.0); // In front of the Temple.
      vector startPoint2 = vector(369.0, 0.00, 131.0); // Behind the Gate.
      vector targetPoint = vector(39.47, 14.45, 21.72); // P1's Temple of the Gods.
      ...
      int routeID2 = kbCreateAttackRouteWithPath("Route To P1 2", startPoint, targetPoint);
      int pathID1 = kbPathCreate("First Attack (Army 1)");
      ...
      int pathID3 = kbPathCreate("First Attack (Army 2)");
      ...
      gLandDefendPlan = createDefendPlan("Primary Land Defend", kbBaseGetMainID(cMyID), 12.0, vector(369.0, 0.00, 131.0), 10);
      ...
      if (elapsed_time_2 != 99999)
      {
         if (attack_buff >= 600 && increase_size == false)
         {
            ...
            if (cDifficultyCurrent == cDifficultyModerate)
            {
               ...
               gMaintainSixthLandUnitAmount *= 1.25;
            ...
            if (cDifficultyCurrent == cDifficultyHard)
            {
               ...
               gMaintainFifthLandUnitAmount *= 2.0;
               gMaintainSixthLandUnitAmount *= 2.0;
               gMaintainSeventhLandUnitAmount *= 1.5;

               data.adjustUnitToMaintainAmount(gThirdLandUnit, gMaintainThirdLandUnitAmount);
               data.adjustUnitToMaintainAmount(gFourthLandUnit, gMaintainFourthLandUnitAmount);
               data.adjustUnitToMaintainAmount(gSixthLandUnit, gMaintainSixthLandUnitAmount);
               data.adjustUnitToMaintainAmount(gSeventhLandUnit, gMaintainSeventhLandUnitAmount);

               gAttackMaxSize *= 1.1; // +10%
               gAttackWave.setMaxAttackSize(gAttackMaxSize);
               gSecondAttackWave.setMaxAttackSize(gAttackMaxSize);
            ...
            if (cDifficultyCurrent == cDifficultyTitan)
            {
               ...
               gMaintainSecondLandUnitAmount *= 2.0;
               gMaintainThirdLandUnitAmount *= 1.5;
               gMaintainFourthLandUnitAmount *= 2.0;
               gMaintainFifthLandUnitAmount *= 2.5;
               gMaintainSixthLandUnitAmount *= 2.0;
               gMaintainSeventhLandUnitAmount *= 2.0;
...
void updateDefendPlans()
{
   ...
   aiPlanSetVariableVector(gLandDefendPlan, cDefendPlanTargetPoint, 0, vector(333.0, 5.0, 259.0));
   aiPlanSetVariableVector(gLandDefendPlan, cDefendPlanGatherPoint, 0, vector(333.0, 5.0, 259.0));
...
// *** TECHS ***
   // MYTHIC AGE
      ...
      // TITAN ONLY:
         // Bravery
            rule researchBravery
            inactive
            minInterval 640
            {
               debugAttackWave("Starting Bravery research plan.");
               researchSimpleTech(cTechBravery, cUnitTypeHillFort, -1, 60);
               xsDisableRule("researchBravery");
            }
         // Iron Shields
