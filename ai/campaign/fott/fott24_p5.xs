...
/* fott24_p5.xs
   ...
   Red Norse player owning a sprawling base of Temples in the east part of the map. Once the player has
   established a base, they send attacks containing Mountain Giants and Frost Giants.

   When the ceaseAttacks() function is called, they stop their attacks. This is used in conjunction with the giant wave being
   launched for their ally, Player 2.
*/
//==============================================================================
// Includes
...
Strategy scenarioAttackWaveStrategy()
{
   ...
   strategy.mInitFunc = [](ref StrategyData data) -> bool
   {
      ...
      vector startPoint = vector(245.0, 1.0, 105.0); // In the eastern mountain path.
      ...
      int pathID1 = kbPathCreate("Path Upper");
      kbPathAddWaypoint(pathID1, startPoint);
      kbPathAddWaypoint(pathID1, vector(264.0, 1.0, 152.0));
      kbPathAddWaypoint(pathID1, targetPoint);
      kbAttackRouteAddPath(routeID, pathID1);

      int pathID2 = kbPathCreate("Path Lower");
      kbPathAddWaypoint(pathID2, startPoint);
      kbPathAddWaypoint(pathID2, vector(180.0, 1.0, 128.0));
      ...
      /*
      // Defend the upper part of the eastern base.
      vector defendPoint = vector(300.0, 2.0, 64.0);
      ...
      // Defend the lower part of the eastern base.
      defendPoint = vector(268.43, 1.53, 58.77);
      ...
      // Keep units in reserve in the eastern base.
      defendPoint = vector(268.43, 1.53, 58.77);
   ...
   // DEFINE THE PLANS
      ...
      gDefendPlan1 = createDefendPlan("Defense Plan 1", kbBaseGetMainID(cMyID), 10, vector(309.0, 0.0, 125.0), 10);
      ...
      gDefendPlan2 = createDefendPlan("Defense Plan 2", kbBaseGetMainID(cMyID), 10, vector(209.0, 0.0, 37.0), 10);
      ...
      gDefendPlan3 = createDefendPlan("Defense Plan 3", kbBaseGetMainID(cMyID), 10, vector(321.0, 0.0, 21.0), 10);
      ...
      gDefendPlan4 = createDefendPlan("Defense Plan 4", kbBaseGetMainID(cMyID), 10, vector(187.0, 0.0, 87.0), 10);
      aiPlanSetVariableFloat(gDefendPlan4, cDefendPlanEngageRange, 0, 30);
      aiPlanAddUnitType(gDefendPlan4, gSecondLandUnit, 0, 0, secondLandUnitSplitAmount); // Frost Giants

      // Plan 5 (Center-East)
      gDefendPlan5 = createDefendPlan("Defense Plan 5", kbBaseGetMainID(cMyID), 10, vector(291.0, 0.0, 67.0), 10);
      aiPlanSetVariableFloat(gDefendPlan5, cDefendPlanEngageRange, 0, 30);
      aiPlanAddUnitType(gDefendPlan5, gFirstLandUnit, 0, 0, firstLandUnitSplitAmount); // Mountain Giants
...
group ruleGroupUpgrades1
{
   // Wait 1000 seconds on Moderate.
   if (cDifficultyCurrent == cDifficultyModerate)
   {
      if (xsGetTime() >= 1000 + gWakeUpTime)
      {
