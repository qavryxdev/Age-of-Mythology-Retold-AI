...
/* fott22_p3.xs
   ...
   fott22 player3
   Continuously trains Frost Giants and Mountain Giants and attacks the player every 4:30 minutes.
*/
//==============================================================================
// Includes
...
int gSecondLandUnit = cUnitTypeMountainGiant; // Gets trained from the start.
float gMaintainSecondLandUnitAmount = 1;
int gExtraLandUnitDelay = 10;

float gAttackStartDelay = 800; // In seconds.
float gAttackWaveInterval = 520; // In seconds.
...
Strategy scenarioAttackWaveStrategy()
{
   ...
   strategy.mInitFunc = [](ref StrategyData data) -> bool
   {
      ...
      vector startPoint = vector(337.0, 0.0, 347.0); // Behind the gate of player 3.
      vector targetPoint = vector(167.0, 0.0, 139.0); // The area near the left most Settlement.
      ...
      kbPathAddWaypoint(pathID1, vector(319.0, 0.0, 371.0)); // 1st Waypoint
      kbPathAddWaypoint(pathID1, vector(243.0, 0.0, 327.0)); // 2nd Waypoint
      kbPathAddWaypoint(pathID1, vector(227.0, 0.0, 245.0)); // 3rd Waypoint
      kbPathAddWaypoint(pathID1, vector(307.0, 0.0, 205.0)); // 4th Waypoint
      kbPathAddWaypoint(pathID1, vector(337.0, 0.0, 81.0)); // 5th Waypoint
      kbPathAddWaypoint(pathID1, vector(211.0, 0.0, 59.0)); // 6th Waypoint
      ...
      kbPathAddWaypoint(pathID2, vector(415.0, 0.0, 291.0)); // 1st Waypoint
      kbPathAddWaypoint(pathID2, vector(307.0, 0.0, 205.0)); // 2nd Waypoint
      kbPathAddWaypoint(pathID2, vector(337.0, 0.0, 81.0)); // 3rd Waypoint
      kbPathAddWaypoint(pathID2, vector(211.0, 0.0, 59.0)); // 4th Waypoint
      kbPathAddWaypoint(pathID2, vector(227.0, 0.0, 245.0)); // 5th Waypoint
      kbPathAddWaypoint(pathID2, vector(167.0, 0.0, 207.0)); // 6th Waypoint
...
// Defend Points (Divided to ensure a more natural distribution of guards)
   ...
   // SPLIT AMOUNTS
      int firstLandUnitSplitAmount = gMaintainFirstLandUnitAmount / 2; // Frost Giants
      int secondLandUnitSplitAmount = gMaintainSecondLandUnitAmount / 2; // Mountain Giants

   // DEFINE THE PLANS
      // Left
      gLeftDefendPlan = createDefendPlan("Left Defense Plan", kbBaseGetMainID(cMyID), 10, vector(315.0, 0.0, 321.0), 20);
      aiPlanSetVariableFloat(gLeftDefendPlan, cDefendPlanEngageRange, 0, 30);
      aiPlanAddUnitType(gLeftDefendPlan, gFirstLandUnit, 0, 0, firstLandUnitSplitAmount); // Frost Giants
      aiPlanAddUnitType(gLeftDefendPlan, gSecondLandUnit, 0, 0, secondLandUnitSplitAmount); // Mountain Giants

      // Right
      gRightDefendPlan = createDefendPlan("Right Defense Plan", kbBaseGetMainID(cMyID), 10, vector(377.0, 0.0, 321.0), 20);
      aiPlanSetVariableFloat(gRightDefendPlan, cDefendPlanEngageRange, 0, 30);
      aiPlanAddUnitType(gRightDefendPlan, gFirstLandUnit, 0, 0, firstLandUnitSplitAmount); // Frost Giants
      aiPlanAddUnitType(gRightDefendPlan, gSecondLandUnit, 0, 0, secondLandUnitSplitAmount); // Mountain Giants
...
//=================================NOT USED=============================================
/*
// Grants Free Favor (this function uses legacy code and needs to be updated eventually)
rule favorDump
active
minInterval 10 // AI will attempt to get the technology every 10 seconds.
{
   // int current_time = xsGetTime();
         ...
         trPlayerGrantResources(3, cResourceFavor, 100);
         return;
}
*/